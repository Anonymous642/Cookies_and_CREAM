    ...
    if (req.func == "get"){
        ...
        if (req.url == ""){  // Set requires only a URL in order to know who the cookie belongs to
            ...
            chrome.cookies.set(details, (cookie) => {
                if (cookie == null){ // If null an error has occured
                    resp({status: -1, message: "Unable to set cookie", error: chrome.runtime.lastError});
                }else{
        ...
        if (req.name == "" || req.url == ""){ // Behaves identically to get
            resp({status: -1, message: "Error: both name and url must be defined"});
             ...
             console.log(details);   
             chrome.cookies.remove(details, (results) => {
                 if (results == null){
                     resp({status: -1, messge: "Unable to remove cookie", error: chrome.runtime.lastError})
                 }else{
