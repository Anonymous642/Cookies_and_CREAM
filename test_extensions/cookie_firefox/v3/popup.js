...
function submit(){
    ...
    (async () => {
        var response = await chrome.runtime.sendMessage({func: selected, name: name, url: url, key: partitionKey, store: store, domain: domain, path: path, expires: expires, value: value, Site: sameSite, secure: secure, http: httpOnly, session: session});
...
function setUpButtons(){
    document.getElementById("permissionButton").addEventListener("click", (event) => {
        ...
        document.getElementById("getAllButton").style.display="inline"; 
