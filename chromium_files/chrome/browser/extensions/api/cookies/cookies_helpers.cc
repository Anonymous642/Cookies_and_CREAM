...
void GetCookieListFromManager(
  ...
  manager->GetCookieList(url, net::CookieOptions::MakeMostInclusive(), // BrowserOnly - update to ignore browseronly 
...
void GetAllCookiesFromManager(
  ...
  manager->GetAllCookiesExt(std::move(callback)); //BrowserOnly - switch to the new extension endpoint
}
