...
// complete.
  ...
  // BrowserOnly - Exposing the keys to steps further along the chain
  const std::string&  get_key() const{
      return URLRequestKey_ -> key();
  }
 ...
 private:
  ...
  void set_cookey(const std::string& cookey){ //BrowserOnly - allow the key to be set by the builder
	  //LOG(INFO) << "COOKEY: " << cookey.c_str();
	  URLRequestKey_ = crypto::SymmetricKey::Import(crypto::SymmetricKey::AES, cookey);
  }
  ...
  std::unique_ptr<crypto::SymmetricKey> URLRequestKey_; //BrowserOnly - declare the key
