...
std::unique_ptr<URLRequestJob> URLRequestHttpJob::Create(URLRequest* request) {
  ...
  if (!url.SchemeIsCryptographic()) {
    ...
    // ERR_CLEARTEXT_NOT_PERMITTED if noto
...
//    if (request() -> url() == "https://172.20.0.6/" ||request() -> url() == "https://172.20.0.7/" || request() -> url() == "https://172.20.0.8/" || request() -> url() == "https://172.20.0.9/" || request() -> url() == "https://172.20.0.10/"){
...
void URLRequestHttpJob::NotifyHeadersComplete() {
  ...
  if (transaction_ && transaction_->IsReadyToRestartForAuth()) {
    ...
    if (request() -> url() == "https://172.20.0.6/" ||request() -> url() == "https://172.20.0.7/" || request() -> url() == "https://172.20.0.8/" || request() -> url() == "https://172.20.0.9/" || request() -> url() == "https://172.20.0.10/"){
      ...
      LOG(INFO) << "TESTING TIMERS: " << stop-startt;
  }
...
void URLRequestHttpJob::StartTransactionInternal() {
  ...
  // BrowserOnly - Get the key, generate the report, and sign
  CookieStore* cookie_store = request() -> context() -> cookie_store();
  const std::string pubkey = request() -> context() -> get_key();
  if (full_cookie_line != ""){ // BrowserOnly - Readd browseronly cookies if any
    request_info_.extra_headers.SetHeader(HttpRequestHeaders::kCookie, full_cookie_line);
    full_cookie_line = "";
  }

  //LOG(INFO) << "KEY: " << base::EscapeAllExceptUnreserved(pubkey);
  //for(auto entry = pubkey.begin(); entry != pubkey.end(); entry++){
	  //LOG(INFO) << "KEY PEICE: " << *entry << " " << unsigned(*entry);
//	  encoded_key << std::setw(2) << std::setfill('0') << std::hex << unsigned(*entry);
 // }
  if (request() -> url().SchemeIsCryptographic()){
      request_info_.extra_headers.SetHeader("CooKey", base::EscapeAllExceptUnreserved(pubkey));
  }
  if(!monitored_cookies_sent.empty()){ // BrowserOnly - generate report and sign  
    std::string report = "";
    for (auto cookie = monitored_cookies_sent.begin(); cookie != monitored_cookies_sent.end(); cookie++){
      std::string cookie_info;
      cookie_store -> GenerateReport(*cookie, &cookie_info);
      report = report + cookie -> Name() + "={" +  cookie_info + "};";
    }
    report = report.substr(0, report.length()-1);
    // BrowserOnly - generate timestamp
    std::string timestamp = std::to_string(base::Time::Now().InSecondsFSinceUnixEpoch());
    if(timestamp.find('.') != std::string::npos){
      timestamp = timestamp.substr(0, timestamp.find_last_not_of('0') + 1);
      if(timestamp.find('.') == timestamp.size()-1){
        timestamp = timestamp.substr(0,timestamp.size()-1);
      }
    }
    report = report + ";T="+timestamp;
    // BrowserOnly - sign the report 
    unsigned char signature[32];
    crypto::HMAC signer(crypto::HMAC::SHA256);
    bool init_success = signer.Init(pubkey);
    bool sign_success = signer.Sign(report, signature, 32); 

    report = report + ";sig=";
    std::stringstream temp;
    //LOG(INFO) << "SIGNATURE: " << signature;
    for (int i=0; i < 32; i++) {
    //  report += static_cast<char>(signature[i]);
      temp << std::setw(2) << std::setfill('0') << std::hex << unsigned(signature[i]);
    }
    report = report + temp.str();
    // BrowserOnly - add the header
    request_info_.extra_headers.SetHeader("Report", report);
    if(false){
        LOG(INFO) << init_success << " " << sign_success;
    }    
    monitored_cookies_sent.clear();
...
void URLRequestHttpJob::SetTesterRecursive(
   ...
   if(count == 0){
       auto stop = std::chrono::high_resolution_clock::now();
       LOG(INFO) << "TESTING TIME: " << stop-start;
       }else{
         CookieStore* cookie_store = request_->context()->cookie_store();
         std::string cookie_string_two = "test=" + std::to_string(count);
         absl::optional<base::Time> server_time_two = absl::nullopt;
         absl::optional<CanonicalCookie> cookie_to_return_two = absl::nullopt;
         CookieInclusionStatus returned_status_two;
         std::unique_ptr<CanonicalCookie> cookie_two = net::CanonicalCookie::Create(url, cookie_string_two, base::Time::Now(), server_time_two, request_->cookie_partition_key(), /*block_truncated=*/true, &returned_status_two);
         CookieAccessResult cookie_access_result_two(returned_status_two);
         cookie_store->SetCanonicalCookieAsync(std::move(cookie_two), url, options, base::BindOnce(&URLRequestHttpJob::SetTesterRecursive, weak_factory_.GetWeakPtr(), options, start, url, count-1, cookie_to_return_two,cookie_string), std::move(cookie_access_result_two));
       }
   }

void URLRequestHttpJob::SetTesterBlank(
   const CookieOptions& options,
   absl::optional<CanonicalCookie> cookie,
   std::string cookie_string,
   CookieAccessResult access_result) {
...
void URLRequestHttpJob::SetTester(
   const CookieOptions& options,
...
void URLRequestHttpJob::GetTester(
   const CookieOptions& options, 
   std::chrono::high_resolution_clock::time_point start,
   ...
   const CookieAccessResultList& excluded_list) {
       auto stop = std::chrono::high_resolution_clock::now();
       LOG(INFO) << "TESTING TIME: " << stop-start;
       //for (auto& c : cookies_with_access_result_list) {
       //    LOG(INFO) << c.cookie.Name();
       //}
   }
...
void URLRequestHttpJob::SetCookieHeaderAndStart(
  ...
  CookieAccessResultList filtered_maybe_included_cookies;
  
  // BrowserOnly - remove browseronly cookies
  CookieAccessResultList browser_only_cookies;
  for (auto& c : maybe_included_cookies) {
      //LOG(INFO) << "TESTING COOKIES " << c.cookie.Name() << " " << c.cookie.IsMonitored();
  
      if (c.cookie.IsBrowserOnly()){
        browser_only_cookies.push_back(c);
      } else {
        filtered_maybe_included_cookies.push_back(c);
      }
  }

  if (!maybe_included_cookies.empty()) {
    std::string cookie_line = "";
    if (!filtered_maybe_included_cookies.empty()){
      cookie_line = 
	      CanonicalCookie::BuildCookieLine(filtered_maybe_included_cookies);
      request_info_.extra_headers.SetHeader(HttpRequestHeaders::kCookie,
                                            cookie_line);
    }

    // BrowserOnly - build the full header for later
    std::string browser_cookie_line = "";
    if (!browser_only_cookies.empty()){
      browser_cookie_line = CanonicalCookie::BuildCookieLine(browser_only_cookies);
      if (cookie_line == "")
        full_cookie_line = browser_cookie_line;
      else
        full_cookie_line = browser_cookie_line + ";" + cookie_line;
    }
    ...
    for (const auto& c : maybe_included_cookies) {
      // BrowserOnly - log monitored cookies for report generation
      if (c.cookie.IsMonitored()){
        monitored_cookies_sent.push_back(c.cookie);
      }
...
void URLRequestHttpJob::SaveCookiesAndNotifyHeadersComplete(int result) {
  ...
  //BrowserOnly - readd browseronly cookies
  for (auto cookie = browser_cookies_received.begin(); cookie != browser_cookies_received.end(); cookie++){
    headers -> AddCookie(*cookie);
  }
  browser_cookies_received.clear();
  ...
  while (headers->EnumerateHeader(&iter, name, &cookie_string)) {
    ...
    //LOG(INFO) << "RECIEVED COOKIE: " << name << " " << cookie_string;

    num_cookie_lines_left_++;
    // BrowserOnly - readd the monitoring information 
    //for (auto cookie : monitored_cookies_received){
      //if (cookie.find(cookie_string.substr(0, cookie_string.find(";"))) != std::string::npos){ 
        //std::string monitored_old = cookie.substr(cookie.find("Montired="), cookie.length());
        //monitored_old = monitored_old.substr(0, monitored_old.find(";"));
        //std::string temp = cookie_string.substr(0, cookie_string.find("Monitored="));
        //cookie_string = temp + monitored_old;
	//break;
      //}
    //}
  ...
  num_cookie_lines_left_--;
  monitored_cookies_received.clear();
...
void URLRequestHttpJob::OnStartCompleted(int result) {
  ...
  if (result == OK) {
    ...
    if (headers -> HasHeader("Set-Cookie")){
      ...
      // BrowserOnly - Parse the recived headers for the two types of cookies
      while(headers -> EnumerateHeader(&iter, "Set-Cookie", &cookie_header)){
        if (cookie_header.find("BrowserOnly") != std::string::npos){
	  browser_cookies_received.push_back(cookie_header);
	}
      } 
      for (auto header : browser_cookies_received){ // BrowseOnly - hide the browseronly cookies
	      headers -> RemoveHeaderLine("Set-Cookie", header);
      }
