...
class NET_EXPORT URLRequestContextBuilder {
 ...
 private:
  ...
  std::unique_ptr<crypto::SymmetricKey> URLRequestKey_ = crypto::SymmetricKey::GenerateRandomKey(crypto::SymmetricKey::AES, 256);
  //std::unique_ptr<crypto::RSAPrivateKey> URLRequestKey_ = crypto::RSAPrivateKey::Create(1024); // BrowserOnly - the key for signing reports
#if BUILDFLAG(ENABLE_REPORTING)
