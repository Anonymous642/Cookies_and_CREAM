...
// BrowserOnly - added monitored data class for the monitored attribute      
MonitoredData::MonitoredData() = default;

MonitoredData::MonitoredData(const MonitoredData& other) = default;

MonitoredData::MonitoredData(MonitoredData&& other) = default;

MonitoredData& MonitoredData::operator=(const MonitoredData& other) = default;

MonitoredData& MonitoredData::operator=(MonitoredData&& other) = default;

MonitoredData::MonitoredData(CookieMonitored level, std::string expected) :
    level_(level), // Level for what to include
    count_(0),  // optional count
    expected_(std::move(expected)) {}

MonitoredData::MonitoredData(CookieMonitored level,
    int count,
...
CanonicalCookie::CanonicalCookie(
    ...
    bool secure,
    bool httponly,
    bool browseronly, // BrowserOnly - introduce browseronly attribute
    CookieSameSite same_site,
    CookiePriority priority,
    absl::optional<CookiePartitionKey> partition_key,
    MonitoredData monitored_data, // BrowserOnly - allow monitored attribute
    ...
    : name_(std::move(name)),
      ...
      secure_(secure),
      httponly_(httponly),
      browseronly_(browseronly),
      same_site_(same_site),
      priority_(priority),
      partition_key_(std::move(partition_key)),
      source_scheme_(source_scheme),
      source_port_(source_port), //{}
      monitored_data_(std::move(monitored_data)) {}
...
std::unique_ptr<CanonicalCookie> CanonicalCookie::Create(
  ...
  *status = CookieInclusionStatus();
  ...
  // BrowserOnly - Create default monitored object and update if cookie has the monitored attribute
  std::string monitored_message = "";
  CookieMonitored monitored_level = parsed_cookie.MonitoredLevel();

  //LOG(INFO) << "BUILDING COOKIE: " << parsed_cookie.Name() << " " << monitored_level << " " << monitored_message;
  if (monitored_level != Cookie_Monitored_NONE) { 
      parsed_cookie.MonitoredMessage(&monitored_message);
      
 }
  MonitoredData monitored_number = MonitoredData(monitored_level, monitored_message);
  ...
  auto cc = std::make_unique<CanonicalCookie>(
      ...
      parsed_cookie.IsHttpOnly(), parsed_cookie.IsBrowserOnly(), samesite, 
      parsed_cookie.Priority(), cookie_partition_key, monitored_number, 
...
std::unique_ptr<CanonicalCookie> CanonicalCookie::CreateSanitizedCookie(
    ...
    bool secure,
    bool http_only,
    //bool browser_only, // BrowserOnly - something is wrong
    CookieSameSite same_site,
    CookiePriority priority,
    absl::optional<CookiePartitionKey> partition_key,
    CookieInclusionStatus* status,
    CookieMonitored monitored_level, // BrowserOnly - individaul monitored parts
  ...
  *status = CookieInclusionStatus();
  ...
  if (!IsCookiePrefixValid(prefix, url, secure, http_only, false, 
			  monitored_level, monitor, domain_attribute, cookie_path)) {  // BrowserOnly - update method call 
    status->AddExclusionReason(
  ...
  // BrowserOnly - make the monitored object
  MonitoredData monitored_number = MonitoredData(monitored_level, monitor);
  
  //LOG(INFO) << "OTHER BUILD COOKIE: " << name << " " << monitored_level << " " << monitor;

  auto cc = std::make_unique<CanonicalCookie>(
      base::PassKey<CanonicalCookie>(), name, value, cookie_domain,
      encoded_cookie_path, creation_time, expiration_time, last_access_time,
      /*last_update=*/base::Time::Now(), secure, http_only, false, 
      same_site, priority, partition_key, monitored_number, source_scheme, 
      source_port);
...
std::unique_ptr<CanonicalCookie> CanonicalCookie::FromStorage(
  ...
  // BrowserOnly - added default monitored object for ease of testing
  auto cc = std::make_unique<CanonicalCookie>(
      base::PassKey<CanonicalCookie>(), std::move(name), std::move(value),
      std::move(domain), std::move(path), creation, expiration, last_access,
      last_update, secure, httponly, false, same_site, priority, partition_key,
      MonitoredData(Cookie_Monitored_NONE, ""), source_scheme, validated_port);
...
std::unique_ptr<CanonicalCookie> CanonicalCookie::CreateUnsafeCookieForTesting(
  ...
  MonitoredData monitored_number; // BrowserOnly - defaulted for ease
  
  return std::make_unique<CanonicalCookie>(
      base::PassKey<CanonicalCookie>(), name, value, domain, path, creation,
      expiration, last_access, last_update, secure, httponly, false, same_site,
      priority, partition_key, monitored_number, source_scheme, source_port);
}
...
CookieAccessResult CanonicalCookie::IncludeForRequestURL(
  ...
  if (options.exclude_browseronly() && IsBrowserOnly())
    status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_BROWSER_ONLY);
...
CookieAccessResult CanonicalCookie::IsSetPermittedInContext(
  ...
  if (options.exclude_browseronly() && IsBrowserOnly()) {
    DVLOG(net::cookie_util::kVlogSetCookies)
        << "BrowserOnly cookie not permitted in script or extension context.";
    access_result.status.AddExclusionReason(
        CookieInclusionStatus::EXCLUDE_BROWSER_ONLY);
  }
...
std::string CanonicalCookie::BuildCookieAttributesLine(
  ...
  if (cookie.IsBrowserOnly()) // BrowserOnly - add attribute to cookie line
    cookie_line += "; browseronly";
...
CanonicalCookie::CookiePrefix CanonicalCookie::GetCookiePrefix(
  ...
  const char kBrowserPrefix[] = "__Browser-"; // BrowserOnly - Create new cookie prefixes
  const char kMonitoredPrefix[] = "__Monitored-";

  base::CompareCase case_sensitivity =
      check_insensitively ? base::CompareCase::INSENSITIVE_ASCII
                          : base::CompareCase::SENSITIVE;

  if (base::StartsWith(name, kBrowserPrefix, case_sensitivity)) // BrowserOnly - check browser only first as it is the most secure
    return CanonicalCookie::COOKIE_PREFIX_BROWSER;
  ...
  if (base::StartsWith(name, kMonitoredPrefix, case_sensitivity))
    return CanonicalCookie::COOKIE_PREFIX_MONITORED;
...
bool CanonicalCookie::IsCookiePrefixValid(CanonicalCookie::CookiePrefix prefix,
  ...
  return CanonicalCookie::IsCookiePrefixValid(
      ...
      parsed_cookie.IsBrowserOnly() , parsed_cookie.MonitoredLevel(),
      monitored_message, 
...
bool CanonicalCookie::IsCookiePrefixValid(CanonicalCookie::CookiePrefix prefix,
                                          ...
                                          bool secure,
					  bool httponly,
					  bool browseronly, // BrowserOnly - add new fields to check on
					  CookieMonitored monitoredLevel,
					  const std::string& monitoredMessage,
                                          const std::string& domain,
                                          const std::string& path) {
  if (prefix == CanonicalCookie::COOKIE_PREFIX_BROWSER) // BrowserOnly -browseronly prefix checks
    return browseronly && httponly && secure && url.SchemeIsCryptographic();
  ...
  if (prefix == CanonicalCookie::COOKIE_PREFIX_MONITORED) { // BrowserOnly - monitored prefix checks
    return monitoredLevel == Cookie_Monitored_HIGH && monitoredMessage != "" && secure;
  }
  return true;
}
...
bool CanonicalCookie::HasHiddenPrefixName(
  ...
  // BrowserOnly - add new prefix to hidden prefix checks
  const base::StringPiece browser_prefix = "__Browser-";
  if (base::StartsWith(value_without_BWS, browser_prefix,
		       base::CompareCase::INSENSITIVE_ASCII)) {
    return true;
  }

  const base::StringPiece monitored_prefix = "__Monitored-";
  
  if (base::StartsWith(value_without_BWS, monitored_prefix,
		       base::CompareCase::INSENSITIVE_ASCII)) {
    return true;
  }
