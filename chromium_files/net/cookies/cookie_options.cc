...
// BrowserOnly - mimic the previous method except for browseronly to make updating apis easier
CookieOptions CookieOptions::MakeMostInclusive() {
  CookieOptions options;
  options.set_include_httponly();
  options.set_exclude_browseronly();
  options.set_same_site_cookie_context(SameSiteCookieContext::MakeInclusive());
  options.set_do_not_update_access_time();
  return options;
}
