...
// BrowserOnly - New labels for the Monitored Attribute
const char kMonitoredNone[] = "none";
const char kMonitoredLow[] = "low";
const char kMonitoredLowCount[] = "lowc"; // optional counting
const char kMonitoredHigh[] = "high";
const char kMonitoredHighCount[] = "highc"; // more oprional counting
...
// BrowserOnly - Methods to convert strings to Monitored class and vice versa
std::string CookieMonitoredStringToString(CookieMonitored monitored){
  switch(monitored){
    case Cookie_Monitored_HIGH_COUNT:
      return kMonitoredHighCount;
    case Cookie_Monitored_HIGH:
      return kMonitoredHigh;
    case Cookie_Monitored_LOW_COUNT:
      return kMonitoredLowCount;
    case Cookie_Monitored_LOW:
      return kMonitoredLow;
    case Cookie_Monitored_NONE:
      return kMonitoredNone;
  }
}

CookieMonitored StringToCookieMonitored(const std::string& monitored){
  std::string monitored_comp = base::ToLowerASCII(monitored);

  if (monitored_comp == kMonitoredHighCount)
    return Cookie_Monitored_HIGH_COUNT;
  if (monitored_comp == kMonitoredHigh)
    return Cookie_Monitored_HIGH;
  if (monitored_comp == kMonitoredLowCount)
    return Cookie_Monitored_LOW_COUNT;
  if (monitored_comp == kMonitoredLow)
     return Cookie_Monitored_LOW;
  
  return Cookie_Monitored_NONE;
}
