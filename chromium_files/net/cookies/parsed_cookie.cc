...
// BrowserOnly - New attribute strings to check for
const char kMonitoredTokenName[] = "monitored";
const char kIsBrowserOnly[] = "browseronly";
...
// BrowserOnly - Converts string into monitored level enum
CookieMonitored ParsedCookie::MonitoredLevel() const {
  return (monitored_index_ == 0) 
	  ? Cookie_Monitored_NONE
	  : StringToCookieMonitored(pairs_[monitored_index_].second.substr(
				  0, pairs_[monitored_index_].second.find("-")));
}

// BrowserOnly - sets monitored message
void ParsedCookie::MonitoredMessage(std::string* output_message) const {
  if (monitored_index_ == 0) 
    *output_message = "";

  *output_message  = pairs_[monitored_index_].second.substr(
	               pairs_[monitored_index_].second.find("-")+1, 
	               pairs_[monitored_index_].second.length());
}
...
// BrowserOnly - new set functions for parser
bool ParsedCookie::SetMonitored(const std::string& monitored_data) {
  return SetString(&monitored_index_, kMonitoredTokenName, monitored_data);
}
bool ParsedCookie::SetBrowserOnly(bool is_browser_only) {
  return SetBool(&is_browser_index_, kIsBrowserOnly, is_browser_only);
}
...
void ParsedCookie::ParseTokenValuePairs(const std::string& cookie_line,
                                        ...
                                        CookieInclusionStatus& status_out) {
  pairs_.clear();
  //LOG(INFO) << "We entered the parsing dimension: " << cookie_line;
...
void ParsedCookie::SetupAttributes() {
  ...
  for (size_t i = 1; i < pairs_.size(); ++i) {
    ...
    } else if (pairs_[i].first == kMonitoredTokenName) { // BrowserOnly - allow new attributes to be found
      monitored_index_ = i;
    } else if (pairs_[i].first == kIsBrowserOnly) {
      is_browser_index_ = i;
    } else {
...
void ParsedCookie::ClearAttributePair(size_t index) {
  ...
  size_t* indexes[] = {
      ...
      &monitored_index_, &is_browser_index_}; // BrowserOnly - new indexe // BrowserOnly - new indexess
