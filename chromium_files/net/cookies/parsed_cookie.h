...
class NET_EXPORT ParsedCookie {
 public:
  ...
  // BrowserOnly - Added attribute checks to the parsed cookie class
  bool IsBrowserOnly() const { return is_browser_index_ != 0; }
  bool IsMonitored() const { return monitored_index_ != 0; }
  CookieMonitored MonitoredLevel() const;
  void MonitoredMessage(std::string* output_message) const;
  ...
  // BrowserOnly - setters for the new attributes
  bool SetMonitored(const std::string& monitored_data);
  bool SetBrowserOnly(bool is_browser_only);
 ...
 private:
  ...
  // BrowserOnly - added indexes to allow paring behavior to work
  size_t monitored_index_ = 0; 
  size_t is_browser_index_ = 0;
