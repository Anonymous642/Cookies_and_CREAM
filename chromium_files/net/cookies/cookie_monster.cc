...
// BrowserOnly - generate the report for the cookie
void CookieMonster::GenerateReport(const CanonicalCookie cc,
		                   std::string* header){

	std::string report = "";
	CookieMonitored monitored_level = cc.MonitoredLevel();

	if (cc.IsCounting()){  //optional 
	  report = report + "\"C\":" + std::to_string(cc.MonitoredCount()) + ",";
	}
	
	if (monitored_level == Cookie_Monitored_HIGH_COUNT || // default for security 
	    monitored_level == Cookie_Monitored_HIGH){
	  std::string log;
	  cc.MonitoredChangelogStr(&log);
	  report = report + "\"CL\":" + log  + ",";
	}
	
	std::string settings;
	cc.MonitoredSettings(&settings);
	report = report + "\"CS\":" + settings;
	std::string expected_mess = cc.MonitoredExpected();
	
	if (expected_mess != ""){ // Again should be there for security
	  report = report + ",\"M\":\"" + expected_mess + "\"";
        }
...
// BrowserOnly - new endpoint for extensions to screen browseronly cookies
void CookieMonster::GetAllCookiesExtAsync(GetAllCookiesCallback callback) {
  DoCookieCallback(base::BindOnce(
      &CookieMonster::GetAllCookiesExt, base::Unretained(this),
      std::move(callback)));
}
...
void CookieMonster::GetAllCookies(GetAllCookiesCallback callback) {
  ...
  for (auto* cookie_ptr : cookie_ptrs){
    if (cookie_ptr -> IsMonitored() && cookie_ptr -> IsCounting()){ // BrowserOnly - optional counting functionality
	    cookie_ptr -> UpdateCount();
    }
...
// BorwserOnly - the actual endpoint. mimics the above call, but filters
//   browseronly cookies
void CookieMonster::GetAllCookiesExt(GetAllCookiesCallback callback) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ...
  cookie_list.reserve(cookie_ptrs.size());
  for (auto* cookie_ptr : cookie_ptrs){
    if (cookie_ptr -> IsBrowserOnly()){ // BrowserOnly - this is how we screen
      continue;
    }
    
    if (cookie_ptr -> IsMonitored() && cookie_ptr -> IsCounting()){ // Again optional counting
      cookie_ptr -> UpdateCount();
    }
    cookie_list.push_back(*cookie_ptr);
  }
...
void CookieMonster::FilterCookiesWithOptions(
  ...
  for (auto& cookie_result : cookies_and_access_results) {
    ...
    if (cookie_ptr -> IsMonitored() && cookie_ptr -> IsCounting()){ // BrowserOnly - optional counting 
	    cookie_ptr -> UpdateCount();
    }
...
void CookieMonster::MaybeDeleteEquivalentCookieAndUpdateStatus(
    const std::string& key,
    CanonicalCookie& cookie_being_set,
    bool allowed_to_set_secure_cookie,
    bool skip_httponly,
    bool skip_browseronly, // BrowserOnly allow ignoring new attribute
  ...
  for (auto cur_it = range_its.first; cur_it != range_its.second; ++cur_it) {
    ...
    if (cookie_being_set.IsEquivalent(*cur_existing_cookie)) {
      ...
      // BrowserOnly - update monitored info 
      if (cur_existing_cookie -> IsMonitored()){
          cookie_being_set.SetCount(cur_existing_cookie -> MonitoredCount()); // optional count
	  cookie_being_set.setMonitoredLevel(cur_existing_cookie -> MonitoredLevel()); // always high for security
          if (cookie_being_set.MonitoredExpected() == "" && cur_existing_cookie -> MonitoredExpected() != ""){ // Carry over expected if not new info sent
	      cookie_being_set.UpdateExpected(cur_existing_cookie -> MonitoredExpected());
	    if (cur_existing_cookie -> MonitoredLevel() == Cookie_Monitored_HIGH_COUNT ||
	        cur_existing_cookie -> MonitoredLevel() == Cookie_Monitored_HIGH){
	        cookie_being_set.SetChangelog(cur_existing_cookie -> MonitoredChangelog());
		std::string difference = "{";

 	        if (cookie_being_set.Value() != cur_existing_cookie -> Value()){
                  difference = difference + "\"V\":\"" + cookie_being_set.Value() + "\",";
                }

                if (cookie_being_set.Domain() != cur_existing_cookie -> Domain()){
                  difference = difference + "\"D\":\"" + cookie_being_set.Domain() + "\",";
                }

                if (cookie_being_set.Path() != cur_existing_cookie -> Path()){
                  difference = difference + "\"P\":\"" + cookie_being_set.Path() + "\",";
                }
 
                if (cookie_being_set.ExpiryDate() != cur_existing_cookie -> ExpiryDate()){
                  difference = difference + "\"E\":" + HttpUtil::TimeFormatHTTP(cookie_being_set.ExpiryDate()) + ",";
                }

                if (cookie_being_set.IsSecure() != cur_existing_cookie -> IsSecure()){
                  difference = difference + "\"S\":";
                  if (cookie_being_set.IsSecure()){
                ...
                if (cookie_being_set.IsHttpOnly() != cur_existing_cookie -> IsHttpOnly()){
                  difference = difference + "\"H\":";
                  if (cookie_being_set.IsHttpOnly()){
                    difference = difference + "1,";
                  }else{
                    difference = difference + "0,";
                  }
                ...
                if (cookie_being_set.IsPartitioned() != cur_existing_cookie -> IsPartitioned()){
                ...
                if (cookie_being_set.SameSite() != cur_existing_cookie -> SameSite()){
                  difference = difference + "\"SS\":";
                  switch (cookie_being_set.SameSite()){
		...
		if (difference != "{"){
		    difference = difference.substr(0, difference.size()-1) + "}";
		    cookie_being_set.UpdateChangelog(difference);
		}
	    }
	 } 
      }
      ...
      if (skip_httponly && cur_existing_cookie->IsHttpOnly()) {
        ...
        status->AddExclusionReason(
            CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY);
      } else if (skip_browseronly && cur_existing_cookie->IsBrowserOnly()){ // BrowserOnly - don't let browseronly be overwritten 
        net_log_.AddEvent(
	  NetLogEventType::COOKIE_STORE_COOKIE_REJECTED_BROWSERONLY,
	  [&](NetLogCaptureMode capture_mode) {
	    return NetLogCookieMonsterCookieRejectedBrowseronly(
	      cur_existing_cookie, &cookie_being_set, capture_mode);
	  });
        status -> AddExclusionReason(
	  CookieInclusionStatus::EXCLUDE_OVERWRITE_BROWSER_ONLY);
      } else {
...
void CookieMonster::SetCanonicalCookie(
  ...
  if (should_try_to_delete_duplicates) {
    MaybeDeleteEquivalentCookieAndUpdateStatus(
        ...
        options.exclude_httponly(), options.exclude_browseronly(), //BrowserOnly - add new option
